...
class QosTestBase(BaseNeutronTest):
    ...
    def validate_control_packet_dscp_marking(self, node_ip,
        ...
        # Building Filter pattern
        filter = []
        if packet_src_ip:
            if isinstance(packet_src_ip, list):
                new_list = ["src host %s" % ip for ip in packet_src_ip]
                new_filter = ["or"] * (len(packet_src_ip)*2-1)
        ...
        if packet_dst_ip:
            if isinstance(packet_dst_ip, list):
                new_list = ["dst host %s" % ip for ip in packet_dst_ip]
                new_filter = ["or"] * (len(packet_dst_ip)*2-1)
                new_filter[0::2] = new_list
                string = " ".join(new_filter)
                filter.append(string)
            else:
                filter.append("dst host %s" % packet_dst_ip )
        if packet_src_port:
            filter.append("src port %s" % packet_src_port)
        if packet_dst_port:
            filter.append("dst port %s" % packet_dst_port)
        ...
        # Getting the interface name correpsonding to fabric interface IP
        cmd = 'netstat -ie | grep -B1 %s | grep -o "^\w*"' % node_ip
        control_interface = self.inputs.run_cmd_on_server(node_ip, cmd)
        # Collecting packets using TCPDump
        session, pcap = start_tcpdump_for_intf(node_ip,\
                username, password, control_interface, \
                filters='-vvxx %s' % filter_string, logger=self.logger)
        sleep(10)
        stop_tcpdump_for_intf(session, pcap, logger=self.logger)
        # Copying file from remote to local node where test is running
        source = "%s@%s:%s" % (username, node_ip, pcap)
        remote_copy(source, "/tmp",src_password=password)
        file_name = "/tmp/" + pcap.split('/')[-1]
        f = open(file_name, 'rb')
        # Processing packtes
        pcap = dpkt.pcap.Reader(f)
        if pcap.readpkts() == []:
            self.logger.info("No packet captured")
            return False
        count = 1
        for ts,buff in pcap:
            ...
            if expected_dscp == actual_dscp:
                self.logger.debug("Validated DSCP marking of %s" % (actual_dscp))
            else:
                self.logger.error("Mismatch between actual and expected DSCP")
                self.logger.error("Expected DSCP: %s, Actual DSCP:%s" %\
                                            (expected_dscp,actual_dscp))
                return False
        f.close()
        self.logger.info("DSCP value on control packet generated by control node"
                         " is as expected.")
        return True
    ...
    def get_control_dscp_values(self):
        '''
        Function to read Global configuration of control traffic DSCP value
        '''
        self.logger.debug("Getting existing control dscp information.")
        global_qos_config_fq_str = ('default-global-system-config:'
                            'default-global-qos-config')
        parent_obj = self.vnc_lib.global_qos_config_read(fq_name_str=
                                                        global_qos_config_fq_str)
        value = parent_obj.get_control_traffic_dscp()
        if value == None:
            self.logger.debug("Control DSCP values not configured")
            control_value, dns_value, analytics_value = None, None, None
        else:
            control_value = value.control
            dns_value = value.dns
            analytics_value = value.analytics
            self.logger.debug("Current control dscp values are as follows \n"
                              "Control dscp value as: %s\n"
                              "Dns dscp value as: %s\n"
                              "Analytics dscp value as: %s" %
                              (control_value, dns_value, analytics_value))
        return control_value, dns_value, analytics_value
    #end get_control_dscp_values

    def set_control_dscp_values(self, control_dscp, dns_dscp, analytics_dscp):
        ''' Function for Global configuration of control traffic DSCP value
        It defines the dscp value of traffic between agent and controller.
        Note that it applies for following:
            1. Control traffic between controller and agent(Bidirectional)
            2. DNS traffic between controller and agent (Bidirectional)
            3. Analytics data from agent to collector (Unidirectional)
        '''
        self.logger.debug("Updating control dscp value as: %s\n"
                          "Updating dns dscp value as: %s\n"
                          "Updating analytics dscp value as: %s" %
                          (control_dscp, dns_dscp, analytics_dscp))
        control_traffic_obj = ControlTrafficDscpType(control=control_dscp,
                                                    analytics=analytics_dscp,
                                                    dns=dns_dscp)
        global_qos_config_fq_str = ('default-global-system-config:'
                            'default-global-qos-config')
        parent_obj = self.vnc_lib.global_qos_config_read(fq_name_str=
                                                        global_qos_config_fq_str)
        parent_obj.set_control_traffic_dscp(control_traffic_obj)
        self.vnc_lib.global_qos_config_update(parent_obj)
    #end set_control_dscp_values

    def get_external_dns_servers(self, node_ip):
        '''
        This function returns the external DNS servers of the host
        When DNS mode is default, the VMs use the DNS servers of the host machine.
        '''
