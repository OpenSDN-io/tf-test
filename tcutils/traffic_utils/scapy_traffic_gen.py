...
class ScapyTraffic(object):
    ...
    For IGMP Header:
        <IGMP version=3 src: '10.10.10.10'  grps:'224.1.1.1' |>
    ...
    def __init__(self, src_vm_fixture, interface=None, interval=1, count=1, mode='L2', **kwargs):
        ...
        self.params['GRE'] = kwargs.get('gre',{})
        self.params['MPLS'] = kwargs.get('mpls',{})
        self.params['INNER_IP'] = kwargs.get('inner_ip',{})
        self.params['INNER_UDP'] = kwargs.get('inner_udp',{})
        self.params['IGMPv3'] = kwargs.get('igmp',{})
        self.igmpv3gr  = kwargs.get('igmpv3gr',{})
        self.params['IGMPv3mr'] = kwargs.get('igmpv3mr',{})
    ...
    def scapy_build_stream(self):
        ...
        for header,dict_values in self.params.items():
            if dict_values:
                ...
                if header == 'IGMPv3mr':
                    records = self.scapy_build_igmpv3gr()
                    records_str =','.join(records)
                    stream = stream + "%s(" % header + header_fields + ', records=['+records_str+'])/'
                else:
                    if header == 'INNER_IP':
                        header = 'IP'
                    elif header == 'INNER_UDP':
                        header = 'UDP'
